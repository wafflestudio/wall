...
trait Auth {
  ...
  def currentUser[Content <: AnyContent](implicit request: { def session:Session }): String = {
    request.session.get("current_user.email").getOrElse("default")
  }
  ...
  def currentUserIdOption[Content <: AnyContent](implicit request: { def session:Session } ) = {
    request.session.get("current_user.id")
  }
  ...
  def AuthenticatedAction(f: Request[AnyContent] => Result): Action[AnyContent] =
    Action { implicit request =>
      if (isSessionTokenValid && request.session.get("current_user.email").isDefined)
        f(request)
      else {
        Logger.info("unauthorized access:" + request.uri)
        Redirect(routes.Application.index).flashing("error" -> "Please log in first to access this url").withSession("redirect_uri" -> request.uri)
      }
    }
...
object Session extends Controller with Login {
