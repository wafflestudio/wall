...
object Group extends Controller with securesocial.core.SecureSocial {
  ...
  def index = SecuredAction { implicit request =>
    val groups = models.User.listGroups(request.user.identityId.id).map(_.frozen)
  ...
  def show(id: String) = SecuredAction { implicit request =>
    ...
    val walls = models.User.listSharedWalls(request.user.identityId.id).map(_.frozen)
  ...
  def create = SecuredAction { implicit request =>
    ...
    val groupId = models.Group.createForUser(name(0), request.user.identityId.id).frozen.id
    models.Group.addUser(groupId, request.user.identityId.id)
    Redirect(routes.Group.show(groupId))
  }

  def addUser(groupId: String) = SecuredAction { implicit request =>
    if (models.Group.isValid(groupId, request.user.identityId.id)) {
      val params = request.body.asFormUrlEncoded.getOrElse[Map[String, Seq[String]]] { Map.empty }
  ...
  def addWallPost(groupId: String) = SecuredAction { implicit request =>
    ...
    if (models.Wall.isValid(wallId(0), request.user.identityId.id)) {
