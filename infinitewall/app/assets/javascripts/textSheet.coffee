...
class window.TextSheet extends Sheet
  ...
  attachSocketAction: () ->
    ...
    @element.on 'setText', (e) ->
      wallSocket.send {action : 'setText', params : {id : self.id, text : self.element.find('div.sheetTextField').html(), keyCode : self.element.find('div.sheetTextField')[0].keyCode, beforeAndAfter : self.element.find('div.sheetTextField')[0].beforeAndAfter}}
